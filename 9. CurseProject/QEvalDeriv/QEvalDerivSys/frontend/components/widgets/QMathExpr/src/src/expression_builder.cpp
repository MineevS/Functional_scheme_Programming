...
ExpressionPtr ExpressionBuilder::factor(WrapperFlags &flags)
{
  ...
  default: {
      //static const QRegularExpression reg_exp = QRegularExpression(QStringLiteral("[A-Z]|[a-z]|[А-я]|[Ёё]"));
...
{
  //static const QRegularExpression reg_exp = QRegularExpression(QStringLiteral("[A-Z]|[a-z]|[А-я]|[Ёё]|[0-9]"));
...
ExpressionPtr ExpressionBuilder::funcName(const QString &name, WrapperFlags &flags, bool needBrackets)
{
  ...
  if (name_to_upper == QStringLiteral("SQRT")) {//QStringLiteral
    ExpressionPtr expr = stringExpression(Wrapper::Expression);
    return new RootExpression(expr);
  }
  else if (name_to_upper == QStringLiteral("SQR")) {//QStringLiteral
    flags.setFlag(WrapperFlag::Powered);
    return makePower(stringExpression(Wrapper::Power), new NumberExpression(2.0));
  }
  else if (name_to_upper == QStringLiteral("LOG")) { //QStringLiteral
    return functionLOG(name, needBrackets);
  }
  else if (name_to_upper == QStringLiteral("ABS")) { //QStringLiteral
    return stringExpression(Wrapper::Abs);
  }
  else if (name_to_upper == QStringLiteral("POW")) { //QStringLiteral
    return functionPOW(flags);
  }
  else if (name_to_upper == QStringLiteral("ROOT")) { //QStringLiteral
    return functionROOT();
  }
  else if (name_to_upper == QStringLiteral("IND")) { //QStringLiteral
    return functionIND();
  }
  else if (name_to_upper == QStringLiteral("LIM")) { //QStringLiteral
    return functionLIM(flags, needBrackets);
  }
  else if (name_to_upper == QStringLiteral("FUNC")) { //QStringLiteral
    return functionFUNC(needBrackets);
  }
  else if (name_to_upper == QStringLiteral("FRAME")) { //QStringLiteral
    return functionFRAME();
  }
  else if (name_to_upper == QStringLiteral("SPACE")) { //QStringLiteral
    return functionSPACE();
  }
  else if (name_to_upper == QStringLiteral("DIFF")) { //QStringLiteral
    return functionDIFF();
  }
  else if (name_to_upper == QStringLiteral("PDIFF")) { //QStringLiteral
    return functionPDIFF();
  }
  else if (name_to_upper == QStringLiteral("DIFFN")) { //QStringLiteral
    return functionDIFFN();
  }
  else if (name_to_upper == QStringLiteral("PDIFFN")) { //QStringLiteral
    return functionPDIFFN();
  }
  else if (name_to_upper == QStringLiteral("DIFFR")) { //QStringLiteral
    return functionDIFFR();
  }
  else if (name_to_upper == QStringLiteral("PDIFFR")) { //QStringLiteral
    return functionPDIFFR();
  }
  else if (name_to_upper == QStringLiteral("DIFFRF")) { //QStringLiteral
    return functionDIFFRF();
  }
  else if (name_to_upper == QStringLiteral("PDIFFRF")) { //QStringLiteral
    return functionPDIFFRF();
  }
  else if (name_to_upper == QStringLiteral("STRING")) { //QStringLiteral
    return functionSTRING();
  }
  else if (name_to_upper == QStringLiteral("STROKES")) { //QStringLiteral
    return functionSTROKES(flags);
  }
  else if (name_to_upper == QStringLiteral("FACT")) { //QStringLiteral
    return functionFACT(flags);
  }
  else if (name_to_upper == QStringLiteral("AT")) { //QStringLiteral
    return functionAT();
  }
  else if (name_to_upper == QStringLiteral("LINE")) { //QStringLiteral
    return functionCAPTION(CapExpression::CapStyle::Line);
  }
  else if (name_to_upper == QStringLiteral("VECT")) { //QStringLiteral
    return functionCAPTION(CapExpression::CapStyle::Vector);
  }
  else if (name_to_upper == QStringLiteral("CAP")) { //QStringLiteral
    return functionCAPTION(CapExpression::CapStyle::Cap);
  }
  else if (name_to_upper == QStringLiteral("TILDA")) { //QStringLiteral
    return functionCAPTION(CapExpression::CapStyle::Tilde);
  }
  else if (name_to_upper == QStringLiteral("POINTS")) { //QStringLiteral
    return functionPOINTS();
  }
  else if (name_to_upper == QStringLiteral("STANDL")) { //QStringLiteral
    return functionSTAND(Qt::AlignLeft);
  }
  else if (name_to_upper == QStringLiteral("STANDC")) { //QStringLiteral
    return functionSTAND(Qt::AlignHCenter);
  }
  else if (name_to_upper == QStringLiteral("STANDR")) { //QStringLiteral
    return functionSTAND(Qt::AlignRight);
  }
  else if (name_to_upper == QStringLiteral("MATRIX")) { //QStringLiteral
    return functionMATRIX();
  }
  else if (name_to_upper == QStringLiteral("SUMMA")) { //QStringLiteral
    return functionGroupOp(GroupType::Summa, needBrackets);
  }
  else if (name_to_upper == QStringLiteral("PROD")) { //QStringLiteral
    return functionGroupOp(GroupType::Product, needBrackets);
  }
  else if (name_to_upper == QStringLiteral("CIRC")) { //QStringLiteral
    return functionGroupOp(GroupType::Circulation, needBrackets);
  }
  else if (name_to_upper == QStringLiteral("INT")) { //QStringLiteral
    return functionGroupOp(GroupType::Integral, needBrackets);
  }
  else if (name_to_upper == QStringLiteral("INTM")) { //QStringLiteral
    return functionGroupOp(GroupType::IntegralM, needBrackets);
  }
  else if (name_to_upper == QStringLiteral("CASE")) { //QStringLiteral
    return functionCASE();
  }
  else if (name_to_upper == QStringLiteral("COMMA")) { //QStringLiteral
    return functionCOMMA();
  }
  else if (name_to_upper == QStringLiteral("BRACKETS")) { //QStringLiteral
    return functionBRACKETS();
  }
  else if (name_to_upper == QStringLiteral("SYSTEM")) { //QStringLiteral
    return functionSYSTEM();
  }
  else if (name_to_upper == QStringLiteral("NUM")) { //QStringLiteral
    return functionNUM();
  }
  else if (name_to_upper == QStringLiteral("SYMBOL")) { //QStringLiteral
    return functionSYMBOL();
  }
  else if (name_to_upper == QStringLiteral("ANGLE")) { //QStringLiteral
    return functionANGLE();
  }
//  else if (name_to_upper == QStringLiteral("COLOR")) {
...
ExpressionPtr ExpressionBuilder::token(const QString &name)
{
  ...
  const QString name_to_upper = name.toUpper();
  if (name_to_upper == QStringLiteral("INF") ) {
    return new SignExpression(SignExpression::SignType::Infinity);
  }
  else if (name_to_upper == QStringLiteral("PLANK")) {
    return new PlanckExpression();
  }
  else if (name_to_upper == QStringLiteral("NIL")
           || name_to_upper == QStringLiteral("NULL") ) {
    return new NullExpression();
  }
  else if (name_to_upper == QStringLiteral("COMMA")) {
    return new CommaExpression();
  }
  else if (name_to_upper == QStringLiteral("CONST")) {
    ExpressionPtr expr = new SimpleExpression(name);
    expr->addNext(new SpaceExpression(3));
    return expr;
  }
  else if (name_to_upper == QStringLiteral("ASTERIX")) {
    return new AsterixExpression();
  }
  else if (name_to_upper == QStringLiteral("MINUS")) {
    return new SignExpression(SignExpression::SignType::Minus);
  }
  else if (name_to_upper == QStringLiteral("PARALLEL")) {
    return new SignExpression(SignExpression::SignType::Parallel);
  }
  else if (name_to_upper == QStringLiteral("PERPENDICULAR")) {
    return new SignExpression(SignExpression::SignType::Perpendicular);
  }
  else if (name_to_upper == QStringLiteral("ANGLE")) {
    return new SignExpression(SignExpression::SignType::Angle);
  }
  else if (name_to_upper == QStringLiteral("EMPTY")) {
    return new EmptyExpression();
  }
  else {
    static const QRegularExpression reg_exp = QRegularExpression("[A-Z]|[a-z]");
...
int ExpressionBuilder::getInt(const QString& str, int from, int& next_pos)
{
  ...
  bool ok;
  const int result = str.mid(from, next_pos - from).toInt(&ok);
  if ( !ok ) {
    next_pos = -1;
...
double ExpressionBuilder::getDouble(const QString &str, int from, int &next_pos)
{
  ...
  bool ok;
  const double result = str.mid(from, next_pos - from).toDouble(&ok);//mid
